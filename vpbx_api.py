...
def save_call_record_mp3(call_id, calling_num, called_num, call_time_ts, call_duration):
    ...
    if response.status_code == 200:
        filename = calling_num + '_'  + called_num + '_' + timestamp_to_date_time(call_time_ts) + '_' + str(call_duration) + '.mp3'
...
def main():
    ...
    for page in range(total_pages):
        ...
        for element in range(elements_on_page):
            ...
            calling_num = call_history[element]['callingNumber'].replace('+7', '8')
            called_num = call_history[element]['calledNumber'].replace('+7', '8')
            ...
            save_call_record_mp3(call_id, calling_num, called_num, call_time_ts, call_duration)
