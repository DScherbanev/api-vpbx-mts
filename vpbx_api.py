...
def save_call_record_mp3(call_id, calling_num, called_num, call_time_ts):
    rec_params = {
        'X-AUTH-TOKEN': VPBX_API_TOKEN,
        'cache-control': 'no-cache'
    }
...
def main():
    ...
    for page in range(total_pages):
        ...
        for element in range(elements_on_page):
            ...
            calling_num = call_history[element]['callingNumber']
            called_num = call_history[element]['calledNumber']
            call_time_ts = call_history[element]['callTime']

            # Вывод информации по конкретному звонку
            # pprint.pprint(call_history[element])

            print(call_id, calling_num, called_num, timestamp_to_date_time(call_time_ts))
            save_call_record_mp3(call_id, calling_num, called_num, call_time_ts)
